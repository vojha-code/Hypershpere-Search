...
CostFunction = @(x) ObjSOP1(x,step,R,Tol_R,Tol_dir);    % Cost Function
...
BestSol = fileBestSol;
pop = fileFinalPop;
nPop = length(pop);        % Population Size
...
for it = 1:MaxIt
    ...
    if (BestSol.Cost < erroTolarance) || (it == MaxIt) || (abs(checkConvergence - it) > 20000)
        disp(['Itr ' num2str(it) ': Best Cost = ' num2str(BestCost(it))]);
        %{
        %Save results
        if (BestSol.Cost < erroTolarance) || (it == MaxIt)
            save("Data/"+exprimentName+"_FinalPop.mat",'pop');
            save("Data/"+exprimentName+"_BestSol.mat",'BestSol');
            save("Data/"+exprimentName+"_BestCost.mat",'BestCost');
