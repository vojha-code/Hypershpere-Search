...
%------------------------------------------------------------------------	
% Displacement monitored (control point)
%------------------------------------------------------------------------	
Ipt = 7;                        % id of the nodes
dir = 3;                        % direction 
Ucp = -5.0;                     % displacement imposed at the control point   
n_step = 1;                     % number of steps of the analysis (at each step Ucp/n_step is imposed
Ipt_gdl = Aff(Ipt,dir);         % degree of freedom monitored

% display indeformed shape
DrawStructure(elements,nodes);

% response vector (used in ObjFunc)
U(n_gdl)=0;
